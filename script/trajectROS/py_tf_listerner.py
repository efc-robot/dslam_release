...
if __name__ == '__main__':  
    ...
    #1. 阻塞直到frame相通
    print '1. 阻塞直到frame相通'  
    listener.waitForTransform("/map1", "/map2", rospy.Time(), rospy.Duration(4.0))
    while not rospy.is_shutdown():  
        try:  
        #2. 监听对应的tf,返回平移和旋转
            print '2. 监听对应的tf,返回平移和旋转'  
            (trans,rot) = listener.lookupTransform("/map1", "/map2", rospy.Time(0)) #rospy.Time(0)不表示0时刻的tf，而是指最近一帧tf 
        ...
        if listener.canTransform("/map1", "/map2",rospy.Time(0)) :
            print 'true'
        else :
            print 'false'
        rate.sleep()
#ifndef _SIFT_FEATURE_H_
#define _SIFT_FEATURE_H_
...
void SIFT_FeatureExtract_ROS(const cv::Mat &image_raw,const  cv::Mat &depth_raw,const  sensor_msgs::CameraInfo camera_info, Size dsize, std::vector<cv::KeyPoint>& keypoints, Mat& descriptor_results, Ptr<FeatureDetector> &detector, Ptr<DescriptorExtractor> &descriptor, sensor_msgs::CameraInfo& camera_info_tmp, Mat& depth_resized, Mat& image_resized);

int SIFT_Match_VO(vector<Point2f>& points_prev, vector<Point2f>& points_curr, Mat& desc_prev, Mat& desc_curr, Mat& depth_prev, Mat& depth_curr, boost::array<double,12> camera_infoP,  Eigen::Quaterniond& q, Mat& t);


void SIFT_FeatureExtract_ROS(const cv::Mat &image_raw,const  cv::Mat &depth_raw,const  sensor_msgs::CameraInfo camera_info, Size dsize, std::vector<cv::KeyPoint>& keypoints, Mat& descriptor_results, Ptr<FeatureDetector> &detector, Ptr<DescriptorExtractor> &descriptor, sensor_msgs::CameraInfo& camera_info_tmp, Mat& depth_resized, Mat& image_resized){
        //裁剪到dsize
    cout << "siftfile 001" << endl;
...
int SIFT_Match_VO(vector<Point2f>& points_prev, vector<Point2f>& points_curr, Mat& desc_prev, Mat& desc_curr, Mat& depth_prev, Mat& depth_curr, const boost::array<double,12> camera_infoP,  Eigen::Quaterniond& q, Mat& t){
    vector<DMatch> matches;
        ...
        for(int i=0; i<matches.size(); i++) {
            if ( matches[i].distance> 200 )
                continue;
            if ( abs(points_prev[matches[i].queryIdx].x-points_curr[matches[i].trainIdx].x)> depth_curr.cols/10 )
                continue;
            if ( abs(points_prev[matches[i].queryIdx].y-points_curr[matches[i].trainIdx].y)> depth_curr.rows/10 )
                continue;
